...
function getTime(date) {
  ...
  const time = new Date(date);
  const hours = checkZero(time.getHours());
  const minutes = checkZero(time.getMinutes());
  const seconds = checkZero(time.getSeconds());
...
function getCountDaysInMonth(month, year) {
  const time = new Date(year, month, 0).getDate();
  return time;
}
...
function formatDate(date) {
  function getAmPmHours(hours) {
    if (hours === 0) {
      return { h: 12, stamp: 'AM' };
    }
    if (hours < 12) {
      return { h: hours, stamp: 'AM' };
    }
    if (hours === 12) {
      return { h: hours, stamp: 'PM' };
    }
    return { h: hours - 12, stamp: 'PM' };
  }

  function formatMinSec(num) {
  ...
  const min = formatMinSec(currentDate.getUTCMinutes());
  const s = formatMinSec(currentDate.getUTCSeconds());

  const correctHours = getAmPmHours(h);

  return `${m}/${d}/${y}, ${correctHours.h}:${min}:${s} ${correctHours.stamp}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= daysInMonth; i += 1) {
    const dayOfWeek = daysOfWeek[new Date(year, month - 1, i).getDay()];
...
function getWeekNumberByDate(date) {
  ...
  const cleanDate = new Date(curYear, 0, 1);
  const msDiff = myDate.getTime() - cleanDate.getTime();
  const weeks = Math.floor(msDiff / (1000 * 60 * 60 * 24 * 7)) + 1;
  return weeks;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDay) {
    for (let i = 0; i < countWorkDays && currentDate <= endDay; i += 1) {
      schedule.push(forDate(currentDate));
      currentDate.setDate(currentDate.getDate() + 1);
    }

    for (let i = 0; i < countOffDays && currentDate <= endDay; i += 1) {
      currentDate.setDate(currentDate.getDate() + 1);
    }
  }
